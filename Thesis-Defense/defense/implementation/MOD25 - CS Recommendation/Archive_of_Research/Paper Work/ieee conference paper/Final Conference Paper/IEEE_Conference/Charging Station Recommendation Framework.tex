...
%-------------------------------------------------------
\begin{abstract}
The rapid growth of electric three-wheelers in Bangladesh has strained urban power infrastructure. This paper presents a simulation-based EV charging station recommendation framework developed using Eclipse SUMO and TraCI. Modeling a realistic urban scenario in Khulna city with a heterogeneous fleet, the proposed multi-criteria decision model monitors vehicle SoC, occupancy, and grid load to provide real-time recommendations. Experimental results show a 90.1\% potential recommendation rate, significantly outperforming the 66\% nearest-only baseline, while effectively reducing congestion. The simulation recorded a peak demand of 1.37 MW, providing a dataset of 33,721 records for future smart grid studies.
\end{abstract}
...
Current research in EV infrastructure planning often utilizes multi-objective optimization and multi-criteria decision-making (MCDM) like AHP and TOPSIS to site stations and manage demand \cite{b1, b2, b5, b6}. Real-time routing and AI-based selection methods, including reinforcement learning, have been explored using simulators like SUMO to balance grid loads and reduce traffic \cite{b7, b10, b12, b17}. Siddiqui et al. \cite{b8} highlighted the importance of feature engineering for forecasting accuracy. However, a gap remains in addressing corridor-limited fleets like South Asian Easy Bikes in an advisory mode that preserves simulation integrity while handling heterogeneous battery capacities \cite{b15}. This paper fills these gaps by providing a calibrated SUMO model and a multi-criteria algorithm evaluated against realistic load-balancing metrics.
...
Based on the vehicle's current SoC , each recommendation event is tagged with one of 3 priority levels 
: CRITICAL (SoC $<$ 20\%), HIGH
...
framework doesn't apply a blanket nearest-station rule even under
