...
\title{A Simulation-Based Multi-Criteria Charging Station Recommendation Framework for
Electric Vehicles Using SUMO and TraCI}
...
Recent studies on planning infrastructure for EVs have relied on multi-objective optimization and MCDM techniques such as AHP and TOPSIS for station location and demand management \cite{b1, b2, b5, b6}. Real-time routing and AI-driven selection strategies such as reinforcement learning were explored via simulation environments such as SUMO to address grid loading problems and minimize traffic \cite{b7, b10, b12, b17}. The significance of feature engineering for forecasting was pointed out by Siddiqui et al. \cite{b8}. Still, there is a need to fill the research gap related to corridor-based fleets such as South Asian Easy Bikes through advising, keeping simulation consistency while accounting for different capacities of the batteries used \cite{b15}. This paper aims to fill those gaps with a simulated SUMO environment and multi-criteria algorithm.
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=0.75\columnwidth]{figures/System_Architecture.png}}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.45\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/Khulna_Highlighted_OSM.png}
    \caption{}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/SUMO_custom_road_network.png}
    \caption{}
  \end{subfigure}
...
interference. Further, each recommendation event is logged as 28 attribute CSV record, generating 1,169 events and a vehicle-centric dataset with 33,721 timestep records.
...
\subsection{Priority classification of recommendations based on SoC of the vehicle}

For every recommendation event generated by the system, the SoC of the vehicle is considered to assign one of the 3 priority levels – CRITICAL (when SoC $<$ 20\%), HIGH (20\% $\leq$ SoC $<$ 30\%) and MEDIUM (30\% $\leq$ SoC $<$ 50\%). Assigning priority level to every recommendation event plays an important role for the following reasons:
\begin{itemize}
  \item \textbf{CRITICAL} (SoC $<$ 20\%): Immediate attention is needed since there is a chance of battery exhaustion during the journey.
  \item \textbf{HIGH} (20\% $\leq$ SoC $<$ 30\%): Urgent attention is required since it needs to charge at the next possible station.
  \item \textbf{MEDIUM} (30\% $\leq$ SoC $<$ 50\%): Proactive recommendation since the vehicle will be able to arrive at the station comfortably.
\end{itemize}
...
Station pa\_8 had the greatest proportion of recommendations (32.4\%), which is attributed to its larger size and location in a central point. All the ten stations had recommendations, indicating that the multi-criteria score-based approach \cite{b7, b12} is capable of distributing demand well. Figure~\ref{fig:entry_exit} shows dynamics in the simulation in real time of station pa\_2, whereby part (a) shows multiple vehicle arrivals at t=126.5 s, while part (b) illustrates the vehicle departures at t=385.0 s.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[b]{0.45\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/entry.PNG}
    \caption{}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/exit.PNG}
    \caption{}
  \end{subfigure}
  \caption{Snapshots of vehicle (a) entry and (b) exit at charging station pa\_2 corridors.}
...
\subsubsection{Feasible Recommendation Rate (FRR)} FRR is the fraction of all recommendation events , where at least one free charging slot was available at the station recommended by the algorithm, at the time of the recommendation.
 A recommendation is “feasible”, if a vehicle is sent to the station can start charging immediately without waiting for a slot to become free.
...
For the baseline, the nearest station is only possible int the 771
events (66.0\% of 1,169) where there was a free slot at the nearest station ,
regardless of diversion. The baseline doesn't attempt to
check or redirect to a better option.

\textbf{Interpretation}: An FRR of 90.1\% means that 90 out of every 100
recommendation events, are taken to a
station where it can immediately plug in. The 24.0 percentage-point
...
\subsubsection{Congestion Avoidance Rate (CAR)} CAR computes the proportion of all recommendation
events , where the proposed framework successfully diverted a vehicle
...
three charging events, which would have overloaded a popular station are
...
charging infrastructure. The baseline's CAR = 0\% confirms that
...
in which the framework successfully moved the vehicle to the
...
\textbf{Interpretation}: A value of HPSDR of 27.9\% shows that the
framework does not simply assume the nearest-station policy
even under high levels of battery urgency but, rather,
reasons about availability and allocates the most critically
affected vehicles to the nearest possible station. A 27.9\%
difference between our framework and the baseline is highly
relevant in the case of Khulna, where being out of battery
midway on a journey means that the Easy Bike or E-Rickshaw
gets stuck and causes traffic as well as income loss for
the operator. Baseline HPSDR is equal to 0\% since there is
no way the baseline framework can diverge from the nearest
station due to the lack of an urgency level parameter.

\subsubsection{Average Distance to Recommended Station} This measure gives
the average straight-line distance (in meters) between a
vehicle’s location at the time of recommendation and the
recommended station. It acts as the cost factor to be
balanced against the benefit gained from congestion avoidance
since routing away from the nearest station increases
travel distance.
...
station. The Euclidean distance is measured using
...
\textbf{Interpretation}: The suggested framework asks vehicles to travel,
on average, 15.8 m further than the nearest station. This is an
accepted cost to avoide crowded stations. To put this
tranasaction in to context, consider that the average SoC at recommendation time is
...
The table~\ref{tab:comparison} combines all four metrics. The proposed
multi-criteria framework outperforms the closest only baseline on every
effectiveness dimension while enduring a small distance
penalty. The combined information is as follows :
...
battery-depleted vehicles receive improved routing --- the framework
applies its highest priority triage precisely, while failures are most
costly in the real world.

\textbf{Avg. Distance (+15.8 m)}: The marginal additional travel introduced
...
A simple network is included in the 600-second simulation window. Full
...
3rd nearest candidate. Dynamic electricity, 
pricing and renewable energy at station nodes will be integrated in future work.
...
recommendation events over 600 seconds. Also it depicts the record of peak
