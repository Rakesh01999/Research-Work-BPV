...
\newcommand{\degreeLineALabel}{This thesis report was submitted in fulfillment of the}
...
type \textit{easyBike} were simulated over a 600-second period, producing 33,721
...
result ($\text{R}^2 \approx 1.00$, $\text{MAE} = 0.0005$). \Gls{lr} also performed
strongly with $\text{R}^2 = 0.9999$ and $\text{MAE} = 1.53$~kW.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{images/dummy-post-horisontal.jpg}
  \caption{High-level research workflow: from SUMO simulation to ML prediction.}
  \label{fig:workflow}
\end{figure}
...
The simulation network consists of a set of bidirectional road edges (E0 through E7)
forming a connected road graph. \Gls{ev} vehicles are spawned at predefined origins and
routed to destinations according to route definitions specified in the route file
(\texttt{Test1\_rou.xml}).
...
Forty-nine \Gls{ev} agents of type \textit{easyBike} were introduced into the simulation.
Key \Gls{ev} parameters include:

\begin{itemize}
  \item Battery capacity: 12,000~Wh (12~kWh)
  \item Initial \Gls{soc}: ranging from approximately 37\% to 55\%
  \item \Gls{soc} charging threshold: 30\% (triggers rerouting)
  \item \Gls{soc} target: 70\% (charging completion target)
  \item Simulation duration: 600 seconds
\end{itemize}
...
The final dataset contains 33,721 records across 25 features. Table~\ref{tab:dataset}
...
\hline
Total records                & 33,721 \\
Number of features           & 25 \\
Unique vehicles              & 49 \\
Simulation duration (s)      & 1 -- 600 \\
Status: Driving              & 18,390 (54.5\%) \\
Status: Charging             & 12,782 (37.9\%) \\
Status: Waiting              & 1,879 (5.6\%) \\
Status: Idle                 & 670 (2.0\%) \\
System load: Minimum (kW)    & 0.00 \\
System load: Mean (kW)       & 963.69 \\
System load: Maximum (kW)    & 1,370.00 \\
System load: Std. Dev. (kW)  & 384.73 \\
\hline
\end{tabular}
\end{center}
\end{table}
...
stations. The mean system load was 963.69~kW, with a peak of 1,370~kW achieved when up
to 46 vehicles were simultaneously active in the network. The standard deviation of
384.73~kW reflects the dynamic nature of vehicle arrivals, departures, and queue
transitions.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{images/dummy-post-horisontal.jpg}
...
\textit{driving} status (54.5\%), followed by \textit{charging} (37.9\%), \textit{waiting}
(5.6\%), and \textit{idle} (2.0\%). This distribution reflects the expected behavior: most
...
Table~\ref{tab:ml_results} summarizes the performance of all four \Gls{ml} models on the
test dataset.
...
XGBoost            & 0.0005 & 0.0012 & 0.9999 \\
Linear Regression  & 1.5339 & 2.0637 & 0.9999 \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{images/dummy-post-horisontal.jpg}
  \caption{Comparison of ML model performance metrics (MAE, RMSE, R\textsuperscript{2}).}
...
$\text{MAE} = 0$, $\text{RMSE} = 0$) on the test set. This near-perfect performance is
attributable to the highly structured and deterministic nature of the simulation data:
the system total load is a precise function of the number of vehicles charging and their
assigned station power ratings. Decision trees are well-suited to capturing such piecewise-
constant relationships, and the ensemble aggregation in \Gls{rf} further stabilizes
predictions \cite{rf_load_forecast2020}.

\subsection{XGBoost}

\Gls{xgb} achieved an \Gls{r2} of 0.9999 and an \Gls{mae} of 0.0005~kW, representing
a near-negligible prediction error. The slight deviation from perfect accuracy is due to
XGBoost's regularization terms, which introduce a small bias to prevent overfitting.
This behavior is desirable in real-world deployment scenarios where generalization to
unseen data is more important than zero training error \cite{xgboost_energy2022}.

\subsection{Linear Regression}

\Gls{lr} achieved the lowest performance of the four models, with an \Gls{mae} of
1.53~kW and an \Gls{rmse} of 2.06~kW, although its \Gls{r2} of 0.9999 still indicates
an excellent fit. The residual error reflects the inherently nonlinear relationship
between features such as \Gls{soc} and system load, which \Gls{lr} cannot fully capture
without explicit polynomial feature engineering \cite{regression_ev2020}.

\section{Discussion of Results}

The results demonstrate several important findings. First, the SUMO simulation framework
successfully generates a rich, \Gls{ml}-ready dataset with sufficient diversity in
vehicle states, station occupancy, and load conditions. The dataset's 25 features capture
both individual vehicle dynamics and system-level emergent behaviors.

Second, the dominance of tree-based models confirms prior findings in the load forecasting
literature \cite{ml_ev_load2021, rf_load_forecast2020}. The deterministic structure of
simulation data makes it particularly amenable to decision tree partitioning, where the
load can be precisely reconstructed from a small number of key features
(\texttt{system\_charging\_vehicles}, \texttt{station\_load\_kW}).

Third, the high \Gls{r2} score of \Gls{lr} (0.9999) suggests that the primary
relationship between features and load is nearly linear at the system level, even though
individual station dynamics are nonlinear. This is consistent with the additive nature of
total load (sum of per-station power contributions) when the number of charging vehicles
...
The simulation generated a 33,721-record dataset with 25 features, capturing vehicle-
...
\Gls{r2} of 0.9999, highlighting the near-linear nature of the aggregate load signal.
...
Several limitations should be noted. The simulation network is a simplified synthetic
road graph that does not represent a specific real-world city, potentially limiting the
generalizability of the trained models to actual deployments. The 600-second simulation
duration, while sufficient for proof-of-concept, represents a very short time horizon
compared to real-world charging demand patterns that span hours or days. Additionally, all
vehicles are of the same type (\textit{easyBike}) with uniform battery capacity, whereas
real-world fleets are heterogeneous. The near-perfect \Gls{ml} scores may also partially
reflect the deterministic structure of simulation data rather than true generalization
capability to real-world stochastic environments.
