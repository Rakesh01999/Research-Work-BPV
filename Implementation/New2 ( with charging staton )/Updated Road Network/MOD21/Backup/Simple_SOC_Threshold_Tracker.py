"""
Simple SOC Threshold Tracker - Updated for Current Configuration
Just tracks when vehicles drop below 30% SOC
No rerouting - just monitoring and reporting
Works with Test1.sumocfg and current charging station setup
"""
...
class SimpleSOCTracker:
    ...
    def run_simulation(self, gui=False, max_time=600):
        ...
        try:
            ...
            while traci.simulation.getMinExpectedNumber() > 0 and step < max_time * 10:
                traci.simulationStep()
                step_sec = traci.simulation.getTime()
                
                # Collect data every second
                if int(step_sec) == step_sec:
                    self.collect_data(int(step_sec))
                
                # Progress update
                if int(step_sec) % 30 == 0 and step_sec > 0:
                    active = len(traci.vehicle.getIDList())
                    violations = len(self.violation_summary)
                    print(f"⏱️  Time: {int(step_sec)}s | Active: {active} | Vehicles below 30%: {violations}")
                
                step += 1
            
            final_time = traci.simulation.getTime()
            print(f"\n✓ Simulation completed at {final_time:.1f}s")
            traci.close()
            return True
    ...
    def export_results(self):
        ...
        with open(report_file, 'w') as f:
            ...
            if self.all_vehicle_data:
                df_all = pd.DataFrame(self.all_vehicle_data)
                f.write(f"Simulation duration: {df_all['timestep_sec'].max()}s\n")
...
def main():
    ...
    MAX_TIME = 600   # 600 seconds = 10 minutes
